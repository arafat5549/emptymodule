...
public class PetResource {
    ...
    private static final String RESPOSE_CODE_405 = "405验证错误";
    ...
    public Response getPetById(
            @ApiParam(value = "你要获取的宠物的ID", allowableValues = "range[1,10]", required = true)
    ...
    public Response deletePet(
            ...
            @ApiParam(value = "要删除的宠物ID", required = true)
            @PathParam("petId") Long petId) {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 400, message = RESPOSE_CODE_400),
            @ApiResponse(code = 404, message = RESPOSE_CODE_404),
            @ApiResponse(code = 405, message = RESPOSE_CODE_405)})
    public Response updatePet(
            @ApiParam(value = "要更新的宠物对象", required = true) Pet pet) {
        Pet updatedPet = petData.addPet(pet);
        return Response.ok().entity(updatedPet).build();
    }
//
